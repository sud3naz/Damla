...
- **Beta caps.** Mainnet purchases are capped at 250 USDC (`DAMLA_MAINNET_MAX_AMOUNT`) and mainnet stays off until `DAMLA_MAINNET_ENABLED=1`, so a prepared treasury key does not expose an unfunded network.
