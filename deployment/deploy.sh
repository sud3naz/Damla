...
ssh "$HOST" 'cd /opt/damla && npm ci --omit=dev --no-audit --no-fund && chown -R damla:damla /opt/damla && install -m 644 deployment/damla.service /etc/systemd/system/damla.service && systemctl daemon-reload && systemctl enable damla && systemctl restart damla && sleep 2 && systemctl --no-pager --lines=5 status damla && curl -s 127.0.0.1:8944/api/health'
